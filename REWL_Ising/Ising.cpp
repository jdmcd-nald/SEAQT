...
const int L1dim = 5;                            // linear dimension
...
const int Eglobalmin = -4*numberspins;          // minimum energy for 2D square lattice Potts model
...
double* microT;
double* microT_buf;
int hist_size = (-Eglobalmin) + 1;        // histogram size
...
int totalenergy()         // returns total energy of system
{
	...
	for (int i = 0; i < numberspins; i++)
	{
		if (bctype == 0)    // periodic boundaries
			for (int j = 0; j < 4; j++)
			{
				if (latticepoint[neighbor[4 * i + j]] == 0)
                    e--;
                if (latticepoint[neighbor[4 * i + j]] == 1)
                    e++;
                //if (latticepoint[neighbor[4 * i + j]]==2)
                  //  e++;
			}
...
int propose_update(int w)    // returns energy change if a spin would be updated
{
	...
	int elocvor = totalenergy();
	int newq = rand() % q;
	latticepoint[w] = newq;    // spin _is_ changed here

	int elocnach = totalenergy();
...
int find_local_energy_range(double Eglobmin, double Eglobmax, double overlap, int N)
{
	...
	if (fopen("Ewindows.dat", "r") == NULL)
	{
        ...
        fprintf(stdoutlog, "Proc %3i: Parameter: Eglobmin -- Eglobmax: %d -- %d; overlap=%i percent, Ewindowwidth=%lf, %s", myid, Emindex, Emaxindex, (int)(overlap * 100.0), Ewidth, ctime(&timenow));
...
int main(int argc, char* argv[])
{
	...
	int iter = 1;
	double check_flatness_every = 10000000;   // in number of sweeps
	int backup;
	int backuptwo;
	...
	while (lnf_slowest > lnfmin)
	{
		...
		// check flatness
		flat = histflat(Eminindex, Emaxindex, 0.99);
