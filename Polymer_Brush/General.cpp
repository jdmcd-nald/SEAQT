...
void solvent_reset(int* p_index, int* c_index) {
  for (int i = reset_indexes[0] + 1; i < reset_indexes[1];
       i++)  // clears the lattice polymer in the range set
  {
...
void init_poly_cores() {
  ...
  poly_lattice_indexes[0] =
      149400 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[1 * lengthpercore] =
      149978 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[2 * lengthpercore] =
      149238 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[3 * lengthpercore] =
      149816 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[4 * lengthpercore] =
      149416 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[5 * lengthpercore] =
      149994 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[6 * lengthpercore] =
      149594 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  poly_lattice_indexes[7 * lengthpercore] =
      150172 - L1dim_S_xy * L1dim_S_xy * 50;  // core definition

  for (int j = 0; j < numbercores; j++) {
    for (int i = 1; i < lengthpercore; i++) {
      poly_lattice_indexes[(j * lengthpercore) + i] =
          poly_lattice_indexes[(j * lengthpercore) + i - 1] -
          L1dim_S_xy * L1dim_S_xy * 2;
  ...
  for (int i = 0; i < numbercores * lengthpercore * pertrusionpercore; i++) {
    ...
    for (int iii = 0; iii < numberneighbors_L; iii++) {
      latticepoint_L[neighbor_L[numberneighbors_L * poly_lattice_indexes[i] +
                                iii]] = 1;
    }
  }
  ...
  //    {
  //        poly_latticepoint_clear(poly_lattice_indexes,
  //        i*lengthpercore*pertrusionpercore,
  //        (1+i)*lengthpercore*pertrusionpercore); // clears all of the lattice
  //        points between the two axis points
  //        stdoutlog = fopen(stdoutlogname, "a");
  //        fprintf(stdoutlog, "base
  ...
  //            {
  //                new_index =
  //                local_en_indexes_op[poly_lattice_indexes[i*lengthpercore*pertrusionpercore+j-1]*120+(randstart+counter)%diff1+74];
  ...
  //                {
  //                    stdoutlog = fopen(stdoutlogname, "a");
  //                    fprintf(stdoutlog, "assigned 1 %i
  ...
  //            {
  //                new_index =
  //                local_en_indexes_op[poly_lattice_indexes[i*lengthpercore*pertrusionpercore+j-1]*120+(randstart+counter)%diff2+74];
  ...
  //                {
  //                    stdoutlog = fopen(stdoutlogname, "a");
  //                    fprintf(stdoutlog, "assigned 2 %i
  //                    %i\n",new_index,assignable);
  //                    fclose(stdoutlog);
  //                }
  //                counter++;
  ...
  //            {
  //                new_index =
  //                local_en_indexes_op[poly_lattice_indexes[i*lengthpercore*pertrusionpercore+j-1]*120+(randstart+counter)%diff3+94];
  //                if(new_index>
  ...
  //                if(assignable) {
  //                    stdoutlog = fopen(stdoutlogname, "a");
  //                        fprintf(stdoutlog, "assigned 3 %i
  //                        %i\n",new_index,assignable);
  //                    fclose(stdoutlog);
  //                }
  //                counter++;
  ...
  //            {
  //                new_index =
  //                local_en_indexes_op[poly_lattice_indexes[i*lengthpercore*pertrusionpercore+j-1]*120+(randstart+counter)%diff4+0];
  //                assignable = latticepoint_assign2(new_index,1);
  //                if(assignable) {
  //                    stdoutlog = fopen(stdoutlogname, "a");
  //                    fprintf(stdoutlog, "assigned 4 %i
  //                    %i\n",new_index,assignable);
  //                    fclose(stdoutlog);
  //                }
  //                counter++;
  ...
  //                stdoutlog = fopen(stdoutlogname, "a");
  //                fprintf(stdoutlog, "couldnt assign
  //                %i,%i,%i\n",i,j,new_index);
  //                fclose(stdoutlog);
  ...
  //                    poly_latticepoint_clear(poly_lattice_indexes,
  //                    i*lengthpercore*pertrusionpercore,
  //                    (1+i)*lengthpercore*pertrusionpercore); // clears all of
  //                    the lattice points between the two axis points
  //                    stdoutlog = fopen(stdoutlogname, "a");
  //                    fprintf(stdoutlog, "ta base
  //                    %i\n",poly_lattice_indexes[i*pertrusionpercore*lengthpercore]);
  //                    fclose(stdoutlog);
  //                    for(int k=1;k<=j;k++)
  //                    {
  ...
  //                        {
  //                            new_index =
  //                            local_en_indexes_op[poly_lattice_indexes[i*lengthpercore*pertrusionpercore+k-1]*120+(randstart+counter)%120+0];
  //                            if(new_index<
  //                            (L1dim_S_xy*L1dim_S_xy*(L1dim_L_z-3)))
  //                            {
  ...
  //                                {
  //                                    assignable =
  //                                    latticepoint_assign2(new_index,1);
  //                                }
  //                            }
  //                            if(assignable) {
  //                                stdoutlog = fopen(stdoutlogname, "a");
  //                                fprintf(stdoutlog, "assigned ta %i
  //                                %i\n",new_index,assignable);
  //                                fclose(stdoutlog);
  //                            }
  //                            counter++;
  ...
  //                            stdoutlog = fopen(stdoutlogname, "a");
  //                            fprintf(stdoutlog, "TA guess assign
  //                            %i,%i,%i\n",i,k,new_index);
  //                            fclose(stdoutlog);
  //                        }
  //                        else{
  ...
  for (int i = 0; i < numbercores * lengthpercore * pertrusionpercore; i++) {
    ...
    if (chain_position == 0)  // core definition needs to be removed or ignored
                              // if there are multiple petrusions from a single
                              // core
    {
      lattice_polymer[3 * poly_lattice_indexes[i] + 1] =
          poly_lattice_indexes[i + 1];
    ...
    } else if (chain_position == (lengthpercore - 1))  // head of the poly chain
    {
      ...
      lattice_polymer[3 * poly_lattice_indexes[i] + 2] =
          poly_lattice_indexes[i - 1];
...
double point_distance(int reference_index_1, int reference_index_2,
                      int(&arr)[3]) {
  //    stdoutlog = fopen(stdoutlogname, "a");
  //    fprintf(stdoutlog, "\t\t\tMade it to point_distance()
  //    reference_index_1:%i
  //    reference_index_2:%i\n",reference_index_1,reference_index_2);
  //    fclose(stdoutlog);
...
double poly_point_distance(int* poly_lattice_coordinates,
                           int polymer_position_1, int polymer_position_2) {
  // stdoutlog = fopen(stdoutlogname, "a");
  // fprintf(stdoutlog, "Made it to poly_point_distance()\n");
  // fclose(stdoutlog);

  int dx = poly_lattice_coordinates[3 * polymer_position_1] -
           poly_lattice_coordinates[3 * polymer_position_2];

  int dy = poly_lattice_coordinates[3 * polymer_position_1 + 1] -
           poly_lattice_coordinates[3 * polymer_position_2 + 1];

  int dz = poly_lattice_coordinates[3 * polymer_position_1 + 2] -
           poly_lattice_coordinates[3 * polymer_position_2 + 2];
...
int general_coord(
    int(&arr)[3],
    int reference_index)  // output the x y and z compenent for any index
{
...
int general_index_op(
    int(&arr)[3])  // output index from any x , y , z components
{
  arr[0] = index_optomize_x[arr[0] + neg_xy];
  arr[1] = index_optomize_y[arr[1] + neg_xy];
  arr[2] = index_optomize_z[arr[2] + neg_z];
  return (calc_index_optomize_x[arr[0]] + calc_index_optomize_y[arr[1]] +
          calc_index_optomize_z[arr[2]]);
}

int poly_latticepoint_clear(int* c_chain, int start,
                            int end)  // set a range of monomers between two to
                                      // zero for unimpeded movement
{
  for (int i = start + 1; i < end; i++) {
    //        stdoutlog = fopen(stdoutlogname, "a");
    //        fprintf(stdoutlog, "Made it to poly_clear()  start: %i  end:
    //        %i\n",start,end);
    //        fprintf(stdoutlog, "c_chain: %i  latticepoint_S[c_chain[i]]:
    //        %i\t",c_chain[i],latticepoint_S[c_chain[i]]);
    //        fprintf(stdoutlog, "neighbor_L[numberneighbors_L * c_chain[i] +
    //        iii]:");
    //        fclose(stdoutlog);
    ...
    for (int iii = 0; iii < numberneighbors_L; iii++) {
      //            stdoutlog = fopen(stdoutlogname, "a");
      //            fprintf(stdoutlog, "c_chain: %i
      //            %i\t",c_chain[i],latticepoint_L[neighbor_L[numberneighbors_L
      //            * c_chain[i] + iii]]);
      //            fclose(stdoutlog);
...
int poly_latticepoint_clear(int index)  // set a range of monomers between two
                                        // to zero for unimpeded movement
{
...
int latticepoint_assign(int index, int spin) {
  //    stdoutlog = fopen(stdoutlogname, "a");
  //    fprintf(stdoutlog, "checking assign index: %i
  ...
  for (int iii = 0; iii < numberneighbors_L; iii++) {
    if (latticepoint_L[neighbor_L[numberneighbors_L * index + iii]] == 0) {
      ...
      //            stdoutlog = fopen(stdoutlogname, "a");
      //            fprintf(stdoutlog,
      //            "ni:%i\t%i\t%i\n",neighbor_L[numberneighbors_L * index +
      //            iii],x,latticepoint_L[neighbor_L[numberneighbors_L * index +
      //            iii]]);
      //            fclose(stdoutlog);

    } else  // resets the points switched to occupaton in the case where
            // occupation is not allowed. Just performs cleanup itself so no
            // other function has to
    {
      //            stdoutlog = fopen(stdoutlogname, "a");
      //            fprintf(stdoutlog, "checking assign index: %i  iii:
      //            %i\n",index,iii);
      //            fprintf(stdoutlog, "b latticepoint_S[index]:
      ...
      //            stdoutlog = fopen(stdoutlogname, "a");
      //            fprintf(stdoutlog, "a latticepoint_S[index]:
      //            %i\n\n",latticepoint_S[index]);
      //            fclose(stdoutlog);
      ...
      for (int j = 0; j < iii; j++) {
        ...
        //                stdoutlog = fopen(stdoutlogname, "a");
        //                fprintf(stdoutlog,
        //                "%i\t%i\n",x,latticepoint_L[neighbor_L[numberneighbors_L
        //                * index + j]]);
        //                fclose(stdoutlog);
      }
...
void lattice_polymer_reset(int* p_chain, int* c_chain, int start,
                           int end)  // retrofit of reorganize function from
                                     // previous code saved some time rewriting
                                     // parts of the function
{
       ...
       i++)  // clears the lattice polymer in the range set
  {
    ...
    lattice_polymer[3 * p_chain[i] + 1] =
        -1;  // next lattice point on chain (if not applicable -1 assigned)
  ...
  for (int i = start + 1; i < end; i++) {
    ...
    {
      lattice_polymer[3 * c_chain[i] + 1] =
          c_chain[i + 1];  // next lattice point on chain (if not applicable -1
                           // assigned)
      lattice_polymer[3 * c_chain[i] + 2] =
          -100;  // previous lattice point on chain (if applicable -1 assigned)
    }
    if (i % lengthpercore != 0 &&
        i % (lengthpercore) != lengthpercore - 1)  // intermediate monomer
    {
      lattice_polymer[3 * c_chain[i] + 1] =
          c_chain[i + 1];  // next lattice point on chain (if not applicable -1
                           // assigned)
      lattice_polymer[3 * c_chain[i] + 2] =
          c_chain[i - 1];  // previous lattice point on chain (if applicable -1
                           // assigned)
    }
    if (i % (lengthpercore) == lengthpercore - 1)  // length of an arm
    {
      lattice_polymer[3 * c_chain[i] + 1] =
          -200;  // next lattice point on chain (if not applicable -1 assigned)
      lattice_polymer[3 * c_chain[i] + 2] =
          c_chain[i - 1];  // previous lattice point on chain (if applicable -1
                           // assigned)
    }
...
int lattice_poly_index_reset(int* p_chain, int* c_chain, int start,
                             int end)  // reorganizes the latticepoints more
                                       // efficient than previous scrubbing over
                                       // entire lattice
{
  for (int i = start + 1; i < end;
       i++)  // clears the lattice polymer in the range set
...
int init_solvent_sub(int index) {
  for (int iii = 0; iii < numberneighbors_L; iii++) {
    if (latticepoint_L[neighbor_L[numberneighbors_L * index + iii]] ==
        0)  // checks to ensure lattice is assignable
    {
...
int init_solvent(int number)  // randomly instance solvent atoms
{
  if (myid < minmaxid) {
    if (fopen("Minimum_Config_Solvent.txt", "r") == NULL) {
      ...
      for (int index = numberspins_S - 1 - L1dim_S_xy - L1dim_S_xy * 3;
           index >= 0; index--)  // sequences through lattice
      {
        if (counter < number && index >= ((upper_bounds)) &&
            index < (L1dim_S_xy * L1dim_S_xy * L1dim_S_z -
                     L1dim_S_xy * L1dim_S_xy)) {  // while all solvent has not
                                                  // been placed and while the
                                                  // solvent is within a given
                                                  // range

          if (latticepoint_S[index] ==
              0)  // checks for if assignment is possible
          {
            latticepoint_S[index] = 2;  // assigns solvent spin

            int neighbor_check =
                0;  // checks if neighbor assignment is possible
            neighbor_check = init_solvent_sub(
                index);  // calls sub function to reduce size of function

            if (neighbor_check == 1)  // if assignment was possible
            {
              solvent_loc[counter] =
                  index;  // assigns the spin to solvent_loc and increments
...
int reduced_lattice_optimization(int index_1, int index_2) {
  ...
  if (shifted_index_2 >= 0 && shifted_index_2 < 7 * L1dim_S_xy * L1dim_S_xy) {
    return lattice_dist_optomize_2_lattice[(shifted_index_1 -
                                            (L1dim_S_xy * L1dim_S_xy * 3)) *
                                               (L1dim_S_xy * L1dim_S_xy * 7) +
                                           shifted_index_2];
  }
...
void init_optomize_energy_index() {
  ...
  for (int a = -3; a <= 3; a++) {
    for (int b = -3; b <= 3; b++) {
      for (int c = -3; c <= 3; c++) {
        int checked_index =
            xyz_index[(a + c_z + 5) * (L1dim_S_xy + 10) * (L1dim_S_xy + 10) +
                      (b + c_y + 5) * (L1dim_S_xy + 10) + (c + c_x + 5)];

        if (point_distance(checked_index, index) < max_en_dist &&
            point_distance(checked_index, index) >= 2) {
          ...
          //                            stdoutlog = fopen(stdoutlogname, "a");
          //                            fprintf(stdoutlog, "%i
          //                            ",neighbor_L[checked_index*numberneighbors_L+i]);
          //                            fclose(stdoutlog);
          //                        }
          //                        stdoutlog = fopen(stdoutlogname, "a");
          //                        fprintf(stdoutlog, "\n");
  ...
  for (int q = 0; q < 120; q++) {
    for (int r = 1; r < numberneighbors_L; r++) {
      ...
      for (int g = 0; g < 120; g++) {
        if (hold_indexes[g] ==
            neighbor_L[hold_indexes[q] * numberneighbors_L + r]) {
  ...
  //            stdoutlog = fopen(stdoutlogname, "a");
  //            fprintf(stdoutlog, "%i
  //            ",what_to_exclude[q*numberneighbors_L+r]);
  //            fclose(stdoutlog);
  //        }
  //
  //        stdoutlog = fopen(stdoutlogname, "a");
...
void optomize() {
  // lattice_dist_optomize = (int*)malloc(L1dim_S_xy * L1dim_S_xy * L1dim_S_z *
  // L1dim_S_xy * L1dim_S_xy * L1dim_S_z * sizeof(int));
  ...
  //        {
  //            lattice_dist_optomize[i*numberspins_S
  //            +j]=int_point_distance(i,j);
  //        }
  //    }

  lattice_dist_optomize_2 =
      (int*)malloc(L1dim_S_xy * L1dim_S_xy * L1dim_S_z * sizeof(int));
  for (int j = 0; j < numberspins_S; j++) {
    float x = j % L1dim_S_xy;                                // column
    float y = (j % (L1dim_S_xy * L1dim_S_xy)) / L1dim_S_xy;  // row

    lattice_dist_optomize_2[j] =
        (x + y * L1dim_S_xy) + L1dim_S_xy * L1dim_S_xy * 3;
  }

  lattice_dist_optomize_2_lattice = (int*)malloc(
      (L1dim_S_xy * L1dim_S_xy) * (L1dim_S_xy * L1dim_S_xy * 7) * sizeof(int));
  for (int i = 0 + (L1dim_S_xy * L1dim_S_xy * 3);
       i < (L1dim_S_xy * L1dim_S_xy * 4); i++) {
    for (int j = 0; j < (L1dim_S_xy * L1dim_S_xy * 7); j++) {
      lattice_dist_optomize_2_lattice[(i - (L1dim_S_xy * L1dim_S_xy * 3)) *
                                          (L1dim_S_xy * L1dim_S_xy * 7) +
                                      j] = int_point_distance(i, j);
    }
  }
  ...
  xyz_index = (int*)malloc((L1dim_S_z + 10) * (L1dim_S_xy + 10) *
  ...
  for (int i = 0; i < (L1dim_S_xy + 10); i++) {
    ...
    for (int j = 0; j < L1dim_S_xy + 10; j++) {
      ...
      for (int k = 0; k < (L1dim_S_z + 10); k++) {
        ...
        xyz_index[k * (L1dim_S_xy + 10) * (L1dim_S_xy + 10) +
                  j * (L1dim_S_xy + 10) + i] = general_index(xyz_array);
        z++;
      }
      y++;
    }
...
void eye() {
  ...
  //            int yes=0;
  //            for(int
  //            ii=0;ii<lengthpercore*pertrusionpercore*numbercores;ii++)
  //            {
  ...
  for (int i = 0; i < lengthpercore * pertrusionpercore * numbercores; i++) {
    if (latticepoint_S[poly_lattice_indexes[i]] != 1) {
      stdoutlog = fopen(stdoutlogname, "a");
      fprintf(stdoutlog,
              "veye i:%i latticepoint_S[poly_lattice_indexes[i]]: %i", i,
              latticepoint_S[poly_lattice_indexes[i]]);
      fclose(stdoutlog);
    ...
    for (int iii = 0; iii < numberneighbors_L; iii++) {
      if (latticepoint_L
              [neighbor_L[numberneighbors_L * poly_lattice_indexes[i] + iii]] !=
          1) {
        int x = latticepoint_L
            [neighbor_L[numberneighbors_L * poly_lattice_indexes[i] + iii]];
    ...
    for (int j = 0; j < lengthpercore * pertrusionpercore * numbercores; j++) {
      if ((reduced_lattice_optimization(poly_lattice_indexes[i],
                                        poly_lattice_indexes[j]) < 4 ||
           poly_lattice_indexes[i] == poly_lattice_indexes[j]) &&
          i != j) {
        float x =
            point_distance(poly_lattice_indexes[i], poly_lattice_indexes[j]);
        int y = int_point_distance(poly_lattice_indexes[i],
                                   poly_lattice_indexes[j]);

        stdoutlog = fopen(stdoutlogname, "a");
        fprintf(stdoutlog,
                "i:%i    j:%i   dist:%i  p_l_i:%i    p_l_i:%i    x:%f   y:%i\n",
                i, j, reduced_lattice_optimization(poly_lattice_indexes[i],
                                                   poly_lattice_indexes[j]),
                poly_lattice_indexes[i], poly_lattice_indexes[j], x, y);
        fclose(stdoutlog);
         ...
         j++)  // calculatw=es energy for solvent interactions
    {
      if ((reduced_lattice_optimization(poly_lattice_indexes[i],
                                        solvent_loc[j]) < 4 ||
           poly_lattice_indexes[i] == solvent_loc[j])) {
        ...
        fprintf(
            stdoutlog,
            "sol i:%i    j:%i   dist:%i  p_l_i:%i    p_l_i:%i    x:%f   y:%i\n",
            i, j, reduced_lattice_optimization(poly_lattice_indexes[i],
                                               solvent_loc[j]),
            poly_lattice_indexes[i], solvent_loc[j], x, y);
        fclose(stdoutlog);
      ...
      if (latticepoint_S[solvent_loc[j]] != 2) {
        stdoutlog = fopen(stdoutlogname, "a");
        fprintf(stdoutlog,
                "sveye i:%i latticepoint_S[poly_lattice_indexes[i]]: %i", i,
                latticepoint_S[solvent_loc[i]]);
        fclose(stdoutlog);

        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      for (int iii = 0; iii < numberneighbors_L; iii++) {
        if (latticepoint_L[neighbor_L[numberneighbors_L * solvent_loc[j] +
                                      iii]] != 2) {
          int x = latticepoint_L[neighbor_L[numberneighbors_L * solvent_loc[j] +
                                            iii]];
          ...
          fprintf(stdoutlog, "sneye iii:%i  i:%i\t %i\t %i\t %i\n", iii, i,
                  neighbor_L[numberneighbors_L * poly_lattice_indexes[i] + iii],
                  x,
                  latticepoint_L[neighbor_L[numberneighbors_L * solvent_loc[i] +
                                            iii]]);
          fclose(stdoutlog);
