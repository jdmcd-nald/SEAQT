...
// in order to get everthing worked out correctly The only changes that should
// be left are verifying current equations Adapting the remaining function
// (should require no change just adaptations) Also running a small test case
...
std::vector<int> checks_counter(30);
long long* lll;

int iterr;  // Apart of an added debug output. Effictively just a counter for
            // the number of repeated histogram values when flatness is checked
int repv = 0;  // Holds the value of the (potentially) repeated minvalue of the
               // histogram
int phrepv;    // Holds the value of the previous minimum histogram value to
               // compare with the prior variable
int minmaxid = 10;

int* poly_lattice_indexes =
    (int*)malloc(numbercores * lengthpercore * pertrusionpercore * sizeof(int));
int* ph_poly_lattice_indexes =
    (int*)malloc(numbercores * lengthpercore * pertrusionpercore * sizeof(int));
int* wl_pseudo_chain =
    (int*)malloc(numbercores * lengthpercore * pertrusionpercore * sizeof(int));

// may not need forgot why this is here in the first place aa it will be tracked
// by the lattice polymer
int* poly_lattice_connection =
    (int*)malloc(numbercores * lengthpercore * pertrusionpercore * sizeof(int));

// hold the physical coordinates of a given monomer
int* poly_lattice_coordinates = (int*)malloc(
    numbercores * lengthpercore * pertrusionpercore * 3 * sizeof(int));
int* ph_poly_lattice_coordinates = (int*)malloc(
    numbercores * lengthpercore * pertrusionpercore * 3 * sizeof(int));
int* wl_pseudo_chain_coordinates = (int*)malloc(
    numbercores * lengthpercore * pertrusionpercore * 3 * sizeof(int));

int reset_solvent_index = 0;
int* solvent_loc =
    (int*)malloc(no_solvent_sites *
                 sizeof(int));  // initializes main function to track solvent
int* ph_solvent_loc =
    (int*)malloc(no_solvent_sites *
                 sizeof(int));  // initializes main function to track solvent
int* wl_solvent_loc =
    (int*)malloc(no_solvent_sites *
                 sizeof(int));  // initializes wl function to track solvent
                                // vector for replacemt if movement fails
...
FILE* DOSEstimate;  // holds values for the dos estimate all values for the rog
                    // and tort are placed here to later be deemed unique or not
char dosestimate[50];
...
int output_choice;
int en_array[4][4]
            [4];  // plan to hold the available energy values of the hamiltonian
                  // will contain 1's for available energies and 0 for others,
                  // made to be a size of three so all values can be looked up
                  // other wise Id have to reduce by 1
int en_array_pp[4][4]
               [4];  // plan to hold the available energy values of the
                     // hamiltonian will contain 1's for available energies and
                     // 0 for others, made to be a size of three so all values
                     // can be looked up other wise Id have to reduce by 1
int en_array_ps[4][4]
               [4];  // plan to hold the available energy values of the
                     // hamiltonian will contain 1's for available energies and
                     // 0 for others, made to be a size of three so all values
...
int en_array_ss[4][4]
               [4];  // plan to hold the available energy values of the
                     ...
                     // 0 for others, made to be a size of three so all values
                     // can be looked up other wise Id have to reduce by 1
int en_array_bond[4][4][4];  // plan to hold the available energy values of the
                             // hamiltonian will contain 1's for available
                             // energies and 0 for others, made to be a size of
                             // three so all values can be looked up other wise
                             // Id have to reduce by 1

int int_en_array[11];  // plan to hold the available energy values of the
                       // hamiltonian will contain 1's for available energies
                       // and 0 for others, made to be a size of three so all
                       // values can be looked up other wise Id have to reduce
                       // by 1
int int_en_array_pp[11];  // plan to hold the available energy values of the
                          // hamiltonian will contain 1's for available energies
                          // and 0 for others, made to be a size of three so all
                          // values can be looked up other wise Id have to
                          // reduce by 1
int int_en_array_ps[11];  // plan to hold the available energy values of the
                          // hamiltonian will contain 1's for available energies
                          // and 0 for others, made to be a size of three so all
                          // values can be looked up other wise Id have to
                          // reduce by 1
int int_en_array_ss[11];  // plan to hold the available energy values of the
                          // hamiltonian will contain 1's for available energies
                          // and 0 for others, made to be a size of three so all
                          // values can be looked up other wise Id have to
                          // reduce by 1
int int_en_array_bond[11];  // plan to hold the available energy values of the
                            // hamiltonian will contain 1's for available
                            // energies and 0 for others, made to be a size of
                            // three so all values can be looked up other wise
                            // Id have to reduce by 1

int en_array_wall[4][4][4];   // plan to hold the available energy values of the
                              // hamiltonian will contain 1's for available
                              // energies and 0 for others, made to be a size of
                              // three so all values can be looked up other wise
                              // Id have to reduce by 1
double dist_array[4][4][4];   // plan to hold the available distances in this
                              // array so the square root does not need to be
                              // called when values are needed
int int_dist_array[4][4][4];  // plan to hold the available distances in this
                              // array so the square root does not need to be
                              // called when values are neededxxxxxx
...
int* lattice_polymer =
    (int*)malloc(((numberspins_S * 3) + 2 + 1) * sizeof(int));
...
double* density_profile_buf =
    (double*)malloc(hist_size * L1dim_S_z * sizeof(double));
double* density_profile =
    (double*)malloc(hist_size * L1dim_S_z * sizeof(double));
double* density_profile_solvent_buf =
    (double*)malloc(hist_size * L1dim_S_z * sizeof(double));
double* density_profile_solvent =
    (double*)malloc(hist_size * L1dim_S_z * sizeof(double));

double* indiv_rog = (double*)malloc(hist_size * numbercores * sizeof(double));
double* indiv_rog_buf =
    (double*)malloc(hist_size * numbercores * sizeof(double));
...
int poly_solvent_mov =
    0;                   // says wether a chosen mov moved solvent or a monomer
int chosen_mov = 0;      // tells which move was last chosen
int poly_mov_start = 0;  // tells which monomwrs were involved with a given move
                         // the starting position (on thr chain)
int poly_mov_end = 0;    // tells which monomer is the end considered loop

int reset_indexes[2] = {0, 0};

int what_to_exclude[120 * numberneighbors_L] = {120};
int* local_en_indexes_op = (int*)malloc(numberspins_S * 120 * sizeof(int));
int local_en_dist[120] = {0};
int local_en_indexes_op_x[120] = {0};
int local_en_indexes_op_y[120] = {0};
int local_en_indexes_op_z[120] = {0};

int* lattice_dist_optomize_2 =
    (int*)malloc(L1dim_S_xy * L1dim_S_xy * L1dim_S_z * sizeof(int));

int* lattice_dist_optomize_2_lattice = (int*)malloc(
    (L1dim_S_xy * L1dim_S_xy) * (L1dim_S_xy * L1dim_S_xy * 7) * sizeof(int));
...
int* xyz_index = (int*)malloc((L1dim_S_z + 10) * (L1dim_S_xy + 10) *
                              (L1dim_S_xy + 10) * sizeof(int));
...
// const int solvent_max = numberspins_S-(
// L1dim_S_xy*L1dim_S_xy*(plane_z_min+1))
// -(L1dim_S_xy*L1dim_S_xy*(L1dim_S_z-plane_z_max)); const int solvent_offset =
// L1dim_S_xy*L1dim_S_xy*(plane_z_min+1);
...
void init_output();
void output_lattice(int index, double rog, double tort,
                    ...
                    double fakeDP[L1dim_S_z], double fakeDPS[L1dim_S_z],
                    int yy);
...
void init_output() {
    ...
    while (fscanf(fptr, "%lf\t%lf\t%lf\n", &value, &valuee, &valueee) > 0) {
        ...
        fprintf(stdoutlog, "init_output2 %i: %f %f %f\n", out_counter,
                output_table[out_counter * 4 + 0],
                output_table[out_counter * 4 + 1],
                output_table[out_counter * 4 + 2]);
        fclose(stdoutlog);
    ...
    while (fscanf(fptr, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", &p1dP, &p2dP, &p3dP,
        ...
        dp_counter--;
        fprintf(stdoutlog,
                "dp points %i: %2.2e %2.2e %2.2e %2.2e %2.2e %2.2e\n",
        ...
        fprintf(stdoutlog, "dpc points %i: %f %f %f %f %f %f\n", dp_counter,
                dp_compare[0 * L1dim_S_z + dp_counter],
                dp_compare[1 * L1dim_S_z + dp_counter],
                dp_compare[2 * L1dim_S_z + dp_counter],
                dp_compare[3 * L1dim_S_z + dp_counter],
                dp_compare[4 * L1dim_S_z + dp_counter],
                dp_compare[5 * L1dim_S_z + dp_counter]);
        fclose(stdoutlog);
    }
    fclose(fptr);
    ...
    double p6dPs;
    while (fscanf(fptr, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", &p1dPs, &p2dPs,
                  &p3dPs, &p4dPs, &p5dPs, &p6dPs) > 0) {
        dps_counter--;
        stdoutlog = fopen(stdoutlogname, "a");

        fprintf(stdoutlog, "dps points %i: %f %f %f %f %f %f\n", dps_counter,
                p1dPs, p2dPs, p3dPs, p4dPs, p5dPs, p6dPs);
        ...
        fprintf(stdoutlog, "dpsc points %i: %f %f %f %f %f %f\n", dps_counter,
                dps_compare[0 * L1dim_S_z + dps_counter],
                dps_compare[1 * L1dim_S_z + dps_counter],
                dps_compare[2 * L1dim_S_z + dps_counter],
                dps_compare[3 * L1dim_S_z + dps_counter],
                dps_compare[4 * L1dim_S_z + dps_counter],
                dps_compare[5 * L1dim_S_z + dps_counter]);
        fclose(stdoutlog);
    }
    fclose(fptr);
...
void output_lattice(int index, double rog, double tort,
    ...
    while (break_statement == 0 && i < table_length) {
        if (((int)(((double)index) * .975)) < output_table[4 * i + 0] &&
            ((int)(((double)index) * 1.025)) > output_table[4 * i + 0]) {
            if (((int)output_table[4 * i + 3]) % 10000 == 0) {
                int check = 1;
                int mercy = 0;
                for (int ii = 0; ii < L1dim_S_z; ii++) {
                    int check2 = 1;
                    if (ii % 2 == 0 && fakeDP[ii] < .002 * 2 &&
                        dp_compare[i * L1dim_S_z + ii] >= .01) {
                        check = 0;
                        check2 = 0;
                    }

                    if (ii % 2 == 0 && fakeDP[ii] >= .01 &&
                        dp_compare[i * L1dim_S_z + ii] < .002 * 2) {
                        check = 0;
                        check2 = 0;
                    }

                    if ((fakeDP[ii] > .002 &&
                         dp_compare[i * L1dim_S_z + ii] > .002) &&
                        (fakeDP[ii] < 0.1 * dp_compare[i * L1dim_S_z + ii] ||
                         (fakeDP[ii]) >
                             1.25 * dp_compare[i * L1dim_S_z + ii])) {
                        check = 0;
                        check2 = 0;
                    }
                    ...
                    //			if(fakeDP[ii] != 0 && fakeDP[ii] >= .01 &&
                    //(fakeDP[ii]<0.5*dp_compare[i*L1dim_S_z+ii] ||
                    //(fakeDP[ii])>1.5*dp_compare[i*L1dim_S_z+ii]))
                    //				{
                    //					check =0;
                    //				}
                }
                ...
                if (check == 1 &&
                    ((int)(((double)rog) * (1 - output_reducing[i]))) <=
                        output_table[4 * i + 1] &&
                    ((int)(((double)rog) * (1 + output_reducing[i]))) >=
                        output_table[4 * i + 1]) {
                    if (((int)(((double)tort) * (1 - output_reducing[i]))) <=
                            output_table[4 * i + 2] &&
                        ((int)(((double)tort) * (1 + output_reducing[i]))) >=
                            output_table[4 * i + 2]) {
                        for (int ii = 0; ii < L1dim_S_z; ii++) {
                            stdoutlog = fopen(stdoutlogname, "a");
                            fprintf(
                                stdoutlog,
                                "utt0 %i: :DP: %2.2e %2.2e :DPS: %2.2e  %2.2e "
                                "bool %i %i \n",
                                ii, fakeDP[ii], dp_compare[i * L1dim_S_z + ii],
                                fakeDPS[ii], dps_compare[i * L1dim_S_z + ii],
                                (ii % 2 == 0 && fakeDP[ii] < .002 &&
                                 dp_compare[i * L1dim_S_z + ii] >= .005),
                                ((fakeDP[ii] <
                                      0.2 * dp_compare[i * L1dim_S_z + ii] ||
                                  (fakeDP[ii]) >
                                      2 * dp_compare[i * L1dim_S_z + ii])));
                            fclose(stdoutlog);
                        }
                        ...
                        int arm = 0;
                        sprintf(filename,
                                "Output_%i_%f_Chain_CCords_Proc%i_%i_%i.data",
                                i, output_table[4 * i + 0], myid,
                                (int)(100000 * output_reducing[i]), number[i]);
                        if ((file = fopen(filename, "w")) == NULL) {
                            ...
                            for (int ii = 0;
                                 ii < numbercores * pertrusionpercore *
                                          lengthpercore;
                                 ii++) {
                                summed_diff[0] = poly_lattice_coordinates
                                    [3 * (arm * lengthpercore)];
                                summed_diff[1] = poly_lattice_coordinates
                                    [3 * (arm * lengthpercore) + 1];
                                summed_diff[2] = poly_lattice_coordinates
                                    [3 * (arm * lengthpercore) + 2];
                        ...
                        int s_array[3] = {0};
                        sprintf(filename,
                                "Output_%i_%f_Chain_CCords_Proc%i_%i_%i.datas",
                                i, output_table[4 * i + 0], myid,
                                (int)(100000 * output_reducing[i]), number[i]);
                        if ((file = fopen(filename, "w")) == NULL) {
                            stdoutlog = fopen(stdoutlogname, "a");
                        ...
                        if (((int)(((double)rog) * (1 - reduced_check))) <=
                                output_table[4 * i + 1] &&
                            ((int)(((double)rog) * (1 + reduced_check))) >=
                                output_table[4 * i + 1]) {
                            if (((int)(((double)tort) * (1 - reduced_check))) <=
                                    output_table[4 * i + 2] &&
                                ((int)(((double)tort) * (1 + reduced_check))) >=
                                    output_table[4 * i + 2]) {
                                output_reducing[i] /= 2;
                                number[i] = 0;
                                output_lattice(index, rog, tort, dx, dy, dz,
                                               fakeDP, fakeDPS, 1);
                            }
                        }
...
void init_neighbors()  // create neighbor list first for smaller lattice
{
    ...
    if (bctype == 0)  // periodic BC
             ...
             i++)  // now treat boundaries separately
        {
            ...
            if ((i % (L1dim_S_xy * L1dim_S_xy)) >
                (L1dim_S * (L1dim_S - 1) - 1))  // bottom row
                neighbor_S[numberneighbors_S * i + 4] =
                    i - (L1dim_S * (L1dim_S - 1));
            // neighbor_S[numberneighbors_S * i] = numberspins_S;
    ...
    for (int i = 0; i < numberspins_L; i++)  // in general
    {
        ...
        int next_plane_ref = ((reference + L1dim_L_xy * L1dim_L_xy) %
                              (L1dim_L_xy * L1dim_L_xy * L1dim_L_z));

        neighbor_L[numberneighbors_L * i] = reference;  // upper topleft
        neighbor_L[numberneighbors_L * i + 1] =
            reference +
            ((reference + 1) % L1dim_L_xy - ((reference) % L1dim_L_xy));
        ;  // upper topright
        neighbor_L[numberneighbors_L * i + 2] =
            (reference + L1dim_L_xy) % (L1dim_L_xy * L1dim_L_xy) +
            (reference / (L1dim_L_xy * L1dim_L_xy)) * L1dim_L_xy *
                L1dim_L_xy;  // upper bottom left
        neighbor_L[numberneighbors_L * i + 3] =
            ((reference + L1dim_L_xy) % (L1dim_L_xy * L1dim_L_xy)) +
            ((reference + 1) % L1dim_L_xy - ((reference) % L1dim_L_xy)) +
            (reference / (L1dim_L_xy * L1dim_L_xy)) * L1dim_L_xy *
                L1dim_L_xy;  // upper bottom right
        neighbor_L[numberneighbors_L * i + 4] =
            next_plane_ref;  // lower top right
        neighbor_L[numberneighbors_L * i + 5] =
            next_plane_ref +
            ((next_plane_ref + 1) % L1dim_L_xy -
             ((next_plane_ref) % L1dim_L_xy));  // lower topright
        neighbor_L[numberneighbors_L * i + 6] =
            (next_plane_ref + L1dim_L_xy) % (L1dim_L_xy * L1dim_L_xy) +
            (next_plane_ref / (L1dim_L_xy * L1dim_L_xy)) * L1dim_L_xy *
                L1dim_L_xy;  // lower bottom left
        neighbor_L[numberneighbors_L * i + 7] =
            ((next_plane_ref + L1dim_L_xy) % (L1dim_L_xy * L1dim_L_xy)) +
            ((next_plane_ref + 1) % L1dim_L_xy -
             ((next_plane_ref) % L1dim_L_xy)) +
            (next_plane_ref / (L1dim_L_xy * L1dim_L_xy)) * L1dim_L_xy *
                L1dim_L_xy;  // lower bottom right
    }
...
void init_lattice(double emin,
                  double emax)  // Changes made to bctype check and made
                                // latticepoint[numbersign equal to 0
{
    stdoutlog = fopen(stdoutlogname, "a");
    ...
    for (int i = 0; i < numberspins_S; i++) {
        ...
        // \n",array[0],array[1],array[2]); fclose(stdoutlog);
    ...
    e = total_energy(poly_lattice_coordinates, poly_lattice_indexes,
                     solvent_loc);

    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog,
            "Proc. %i: Initialized lattice with energy e=%i, create setup with "
            "%lf<e<%lf\n",
            myid, e, (emin + (emax - emin) / 3),
            (emin + 2 * (emax - emin) / 3));
    fclose(stdoutlog);
    ...
    for (int i = 0; i < numberspins_S; i++) {
        ...
        if ((i + 1) % L1dim_S_xy == 0) fprintf(stdoutlog, "\n");
        if ((i + 1) % (L1dim_S_xy * L1dim_S_xy) == 0) fprintf(stdoutlog, "\n");
    }
    fprintf(stdoutlog, "\n");
    fprintf(stdoutlog, "\n");
    fclose(stdoutlog);
    ...
    for (int i = 0; i < numberspins_L; i++) {
        ...
        if ((i + 1) % L1dim_L_xy == 0) fprintf(stdoutlog, "\n");
        if ((i + 1) % (L1dim_L_xy * L1dim_L_xy) == 0) fprintf(stdoutlog, "\n");
    }
    ...
    for (int i = 0; i < numbercores * lengthpercore * pertrusionpercore;
         i++)  // defines wl pseudo_chain for use in the below local energy
               // calculation
    {
        wl_pseudo_chain[i] = poly_lattice_indexes[i];
        wl_pseudo_chain_coordinates[3 * i] = poly_lattice_coordinates[3 * i];
        wl_pseudo_chain_coordinates[3 * i + 1] =
            poly_lattice_coordinates[3 * i + 1];
        wl_pseudo_chain_coordinates[3 * i + 2] =
            poly_lattice_coordinates[3 * i + 2];
    }
    ...
    ////        if(increm == 685)
    ////        {
    ////            fprintf(stdoutlog, "686 1 latticepoint 3233
    ///%i\n",latticepoint_L[3233]); /        } /        fclose(stdoutlog);
    //
    //      // ep =
    //      total_energy(poly_lattice_coordinates,poly_lattice_indexes,solvent_loc);
    //      // compares the energy before and after a movement has been
    //      preformed
    //
    //        poly_mov();
    //
    ...
    //
    //       //e =
    //       total_energy(ph_poly_lattice_coordinates,ph_poly_lattice_indexes,ph_solvent_loc);
    //
    //
    //
    //        loc_energy_before = 0;
    ...
    //                                  {
    //                                      //lattice_polymer_reset(poly_lattice_indexes
    //                                      , ph_poly_lattice_indexes,
    //                                      reset_indexes[0], reset_indexes[1]);
    //                                      lattice_poly_index_reset(poly_lattice_indexes,
    //                                      ph_poly_lattice_indexes,
    //                                      reset_indexes[0], reset_indexes[1]);
    //                                      poly_coord_reset(poly_lattice_coordinates,
    //                                      ph_poly_lattice_coordinates,
    //                                      reset_indexes[0], reset_indexes[1]);
    //
    //                                  }
    ...
    //
    //                      //                stdoutlog = fopen(stdoutlogname,
    //                      "a");
    //                      //                fprintf(stdoutlog,
    //                      "solvent_loc[i]: %i ph_solvent_loc[i]:
    ...
    //        loc_energy_after =
    //        local_energy(ph_poly_lattice_coordinates,ph_poly_lattice_indexes,ph_solvent_loc);
    //        }
    //        increm = increm + 1;
    //
    //
    ...
    ///e:%i   ep-e: %i\n",increm%1,ep,e,ep-e); /        fprintf(stdoutlog,
    ///"local_energy_before: %i   local_energy_after: %i   loc_en_b-loc_en_a:
    ...
    ///poly_solv:%i\n",chosen_mov,poly_solvent_mov); / fprintf(stdoutlog,
    ///"reset_indexes[0]+1:%i \t
    ...
    ///%i\n",cccounter,MoveProposal); /            fclose(stdoutlog); / eye();
    ////                        MPI_Abort(MPI_COMM_WORLD, 1);
    ////       }
    //
    ////        if(min_e>ep)
    ...
    ////            fprintf(stdoutlog, "%i\n", e);
    ////            fprintf(stdoutlog, "\n");
    ////            for (int i = 0; i < numbercores * lengthpercore *
    ///pertrusionpercore; i++) /            { / fprintf(stdoutlog, "i: %i
    ///%4i\t\t%4i\t%4i\t%4i\t \t%4i\t\t%4i\t%4i\t%4i\n",i,
    ///poly_lattice_indexes[i], poly_lattice_coordinates[3 * i],
    ///poly_lattice_coordinates[3 * i + 1], poly_lattice_coordinates[3 * i +
    ///2],ph_poly_lattice_indexes[i], ph_poly_lattice_coordinates[3 * i],
    ///ph_poly_lattice_coordinates[3 * i + 1], ph_poly_lattice_coordinates[3 * i
    ///+ 2]); /            }
    //////
    ////            fprintf(stdoutlog, "\n");
    ...
    int ee = total_energy(ph_poly_lattice_coordinates, ph_poly_lattice_indexes,
                          ph_solvent_loc);
    eye();
    if (e != ee) {
        stdoutlog = fopen(stdoutlogname, "a");
    ...
    // Pseudo WL Process is started for all processes to explore the energy
    // levels
    stdoutlog = fopen(stdoutlogname, "a");
    ...
    if (multiple > 1 &&
        merge_hists == 1)  // merge g(E) estimators from multiple walkers in the
                           // same energy window
    {
        stdoutlog = fopen(stdoutlogname, "a");
        if (myid % multiple == 0)  // 'root' in energy window, receive
                                   // individual g(E) and send merged g(E)
        {
            for (int i = 1; i < multiple; i++) {
                MPI_Recv(&lngE_buf[0], hist_size, MPI_DOUBLE, myid + i, 76,
                         MPI_COMM_WORLD, &status);  // get other dens. of states
                fprintf(stdoutlog, "Proc %i: Received lngE from Proc. %i\n",
                        myid, myid + i);
                for (int j = 0; j < hist_size; j++)
                    lngE[j] += lngE_buf[j];  // sum up for average
            }
            for (int i = 1; i < multiple; i++) {
                MPI_Send(&lngE[0], hist_size, MPI_DOUBLE, myid + i, 98,
                         MPI_COMM_WORLD);
        ...
        {
            MPI_Send(&lngE[0], hist_size, MPI_DOUBLE, myid - (myid % multiple),
                     76, MPI_COMM_WORLD);
            fprintf(stdoutlog, "Proc %i: Sent lngE to Proc. %i\n", myid,
                    myid - (myid % multiple));
            MPI_Recv(&lngE_buf[0], hist_size, MPI_DOUBLE,
                     myid - (myid % multiple), 98, MPI_COMM_WORLD, &status);
            fprintf(stdoutlog,
                    "Proc %i: Received combined lngE from Proc. %i\n", myid,
                    myid - (myid % multiple));
            for (int j = 0; j < hist_size; j++)
                lngE[j] = lngE_buf[j];  // replace individual lngE (could be
                                        // done directly, yes)
        }
        fclose(stdoutlog);
    }
...
void init_hists()  // initialize histograms
{
    ...
    pseudolngE = (double*)malloc(
        hist_size * sizeof(double));  // added for pseudo wl process
...
int find_local_energy_range(double Eglobmin, double Eglobmax, double overlap,
                            int N) {
    stdoutlog = fopen(stdoutlogname, "a");
    ...
    if (N % multiple != 0) {
        fprintf(stdoutlog,
                "Total number of processes (%i) must be a multiple of the "
                "number of walkers per energy range (%i)!\n",
                N, multiple);
        fclose(stdoutlog);
        return (1);
    }
    ...
    // fprintf(stdoutlog, "\nProc %i: Can't find file Ewindows.dat. Will
    // calculate equal-size windows with overlap %lf\n", myid, overlap);
    // double Ewidth = (Eglobmax - Eglobmin) / (1.0 + ((double)(N / multiple)
    // - 1.0) * (1.0 - overlap));
    ...
    if (fopen("Ewindows.dat", "r") == NULL) {
        fprintf(stdoutlog,
                "\nProc %i: Can't find file Ewindows.dat. Will calculate "
                "equal-size windows with overlap %lf\n",
                myid, overlap);
        double Ewidth =
            (Eglobmax - Eglobmin) /
            (1.0 + ((double)(N / multiple) - 1.0) * (1.0 - overlap));
        ...
        time(&timenow);
        fprintf(stdoutlog,
                "Proc %3i: Parameter: Eglobmin -- Eglobmax: %lf -- %lf; "
                "overlap=%i percent, Ewindowwidth=%lf, %s",
                myid, Eglobmin, Eglobmax, (int)(overlap * 100.0), Ewidth,
                ctime(&timenow));
        // fprintf(stdoutlog, "Proc %3i: Parameter: Eglobmin -- Eglobmax: %lf --
        // %lf; %s", myid, Emin, Emax, ctime(&timenow));
    } else  // rewrite of prior file reading system to be more portable
    {
        FILE* fptr;
        int value;
...
int replica_exchange(int swap_direction, int index_akt) {
    ...
    swap_direction =
        swap_direction % 2;  // comes actually as number of swap attempt
    ...
    if (mylocalid[swap_direction] ==
        0)  // 'head-node' in the energy window determines pairs of flippartners
    {
        // stdoutlog = fopen(stdoutlogname, "a");
        // fprintf(stdoutlog, "2\n");
        // fclose(stdoutlog);

        int choose_from = multiple;  // number of free partners in higher window
                                     // of communicator
        ...
        for (int i = 0; i < multiple; i++)
            libre[i] = multiple + i;  // initialise

        // idea: processes from the lower window choose someone from the higher
        // window at random of course, the chosen walker can't have an exchange
        // partner yet
        for (int i = 0; i < multiple;
             i++)  // loop over processes in the lower window
        {
            ...
            // update list
            choose_from--;
            for (int j = select; j < choose_from; j++) libre[j] = libre[j + 1];
        }

        //       stdoutlog=fopen(stdoutlogname,"a");
        //       fprintf(stdoutlog,"Proc %3i: Drew the following swap
        //       partners:\n",myid); for (int i=0;i<2*multiple;i++)
        //     fprintf(stdoutlog,"Proc %3i: %i -- %i (local ids in
        //     communicator)\n",myid,i,pairs[i]);
        //       fclose(stdoutlog);
    ...
    // at this point, every walker has a swap partner assigned, now they must be
    // communicated
    if ((swap_direction == 0) &&
        (myid < (numprocs -
                 multiple)))  // the walkers from the last node should not swap
    {
        comm_id =
            2 * (myid / (2 * multiple));  // ! all integer, the '/' is a (div) !
                                          // Not the same as myid/multiple !
        MPI_Scatter(pairs, 1, MPI_INT, &swap_partner, 1, MPI_INT, 0,
                    mpi_local_comm[comm_id]);
    }

    if ((swap_direction == 1) &&
        (myid >= multiple))  // the walkers from the zero-node should not swap
    {
        comm_id = ((myid - multiple) / (2 * multiple)) * 2 +
                  1;  // ! all integer, the '/' is a (div) ! See above
        MPI_Scatter(pairs, 1, MPI_INT, &swap_partner, 1, MPI_INT, 0,
                    mpi_local_comm[comm_id]);
    }
    ...
    if (swap_partner !=
        -1)  // i.e. if there is a swap-partner for me (if I am at a boundary, I
             // might not have a swap partner this time)
    {
        // stdoutlog = fopen(stdoutlogname, "a");
        // fprintf(stdoutlog, "3\n");
        // fclose(stdoutlog);
        ...
        // statistics
        if (swap_partner > mylocalid[swap_direction])
            tryright++;
        else
            tryleft++;
        ...
        //        {
        //            stdoutlog = fopen(stdoutlogname, "a");
        //            fprintf(stdoutlog, "Proc %3i, replica_exchange():
        //            Something is wrong here! Received index=%i, calculated
        //            index=%i totalenergy=%i. Abort.\n", myid, index_akt, Ecur
        //            + (-Eglobalmin), total_energy()); fclose(stdoutlog);
        //            MPI_Abort(MPI_COMM_WORLD, 1);
        //        }
        ...
        MPI_Sendrecv_replace(&i_new, 1, MPI_INT, swap_partner, 1, swap_partner,
                             1, mpi_local_comm[comm_id], &status);

        if ((i_new > Emaxindex) ||
            (i_new < Eminindex))  // energyranges must overlap!
        {
            myfrac = -1.0;
        } else {
        ...
        if (mylocalid[swap_direction] <
            multiple)  // I am receiver and calculator
        {
            // get my partners part of the exchange probability
            MPI_Recv(&otherfrac, 1, MPI_DOUBLE, swap_partner, 2,
                     mpi_local_comm[comm_id], &status);
            ...
            MPI_Send(&change, 1, MPI_INT, swap_partner, 3,
                     mpi_local_comm[comm_id]);
        } else  // I just send my part of exchange probability and await
                // decision
        {
            MPI_Send(&myfrac, 1, MPI_DOUBLE, swap_partner, 2,
                     mpi_local_comm[comm_id]);
            MPI_Recv(&change, 1, MPI_INT, swap_partner, 3,
                     mpi_local_comm[comm_id], &status);
        }
        ...
        if (change == 1) {
            ...
            MPI_Sendrecv_replace(
                ...
                numbercores * lengthpercore * pertrusionpercore, MPI_INT,
                swap_partner, 20, swap_partner, 20, mpi_local_comm[comm_id],
                &status);
            MPI_Sendrecv_replace(
                &poly_lattice_coordinates[0],
                numbercores * lengthpercore * pertrusionpercore * 3, MPI_INT,
                swap_partner, 10, swap_partner, 10, mpi_local_comm[comm_id],
                &status);
            MPI_Sendrecv_replace(&solvent_loc[0], no_solvent_sites, MPI_INT,
                                 swap_partner, 30, swap_partner, 30,
                                 mpi_local_comm[comm_id], &status);
...
void sysparam(int index) {
    ...
    double rsum_z[numbercores * pertrusionpercore] = {0};
    double rsum_xy[numbercores * pertrusionpercore] = {0};
    double radius = 0;
    double radius_z = 0;
    double radius_xy = 0;

    //          summed_diff[0]=poly_lattice_coordinates[3 *
    //          (arm*lengthpercore)];
    //            summed_diff[1]=poly_lattice_coordinates[3 *
    //            (arm*lengthpercore) + 1];
    //            summed_diff[2]=poly_lattice_coordinates[3 *
    //            (arm*lengthpercore) + 2];
    //
    //            summed_diff[0]+=dx[i];
    //            summed_diff[1]+=dy[i];
    ...
    for (int i = (0); i < numbercores * pertrusionpercore;
         i++)  // performs the rotations
    {
        double centerofmass[3] = {0};
        s_dx[i * lengthpercore] =
            poly_lattice_coordinates[3 * (i * lengthpercore)];
        s_dy[i * lengthpercore] =
            poly_lattice_coordinates[3 * (i * lengthpercore) + 1];
        s_dz[i * lengthpercore] =
            poly_lattice_coordinates[3 * (i * lengthpercore) + 2];
        ...
        {
            dx[i * lengthpercore + j] =
                poly_lattice_coordinates[3 * (i * lengthpercore + j)] -
                poly_lattice_coordinates[3 * (i * lengthpercore + j - 1)];

            dy[i * lengthpercore + j] =
                poly_lattice_coordinates[3 * (i * lengthpercore + j) + 1] -
                poly_lattice_coordinates[3 * (i * lengthpercore + j - 1) + 1];

            dz[i * lengthpercore + j] =
                poly_lattice_coordinates[3 * (i * lengthpercore + j) + 2] -
                poly_lattice_coordinates[3 * (i * lengthpercore + j - 1) + 2];

            //            stdoutlog = fopen(stdoutlogname, "a");
            //            fprintf(stdoutlog, "d={%i,%i,%i}
            //            d-1={%i,%i,%i}\n\tp_l_c_diff={%i,%i,%i}\n",dx[i+j],dy[i+j],dz[i+j],dx[i+j-1],dy[i+j-1],dz[i+j-1],poly_lattice_coordinates[3
            //            * (i+j)]-poly_lattice_coordinates[3 * (i+j-1)]
            //            ,poly_lattice_coordinates[3 * (i+j)+1]
            //            -poly_lattice_coordinates[3 * (i+j-1) +
            //            1],poly_lattice_coordinates[3 *
            //            (i+j)+2]-poly_lattice_coordinates[3 * (i+j-1)+2]);
            //            fclose(stdoutlog);

            dx[i * lengthpercore + j] =
                dx[i * lengthpercore + j - 1] -
                (dx[i * lengthpercore + j] -
                 anint(dx[i * lengthpercore + j], L1dim_S_xy) * L1dim_S_xy);
            dy[i * lengthpercore + j] =
                dy[i * lengthpercore + j - 1] -
                (dy[i * lengthpercore + j] -
                 anint(dy[i * lengthpercore + j], L1dim_S_xy) * L1dim_S_xy);
            dz[i * lengthpercore + j] =
                dz[i * lengthpercore + j - 1] -
                (dz[i * lengthpercore + j] -
                 anint(dz[i * lengthpercore + j], L1dim_S_z) * L1dim_S_z);

            s_dx[i * lengthpercore + j] +=
                s_dx[i * lengthpercore] + dx[i * lengthpercore + j];
            s_dy[i * lengthpercore + j] +=
                s_dy[i * lengthpercore] + dy[i * lengthpercore + j];
            s_dz[i * lengthpercore + j] +=
                s_dz[i * lengthpercore] + dz[i * lengthpercore + j];
            ...
            //                fprintf(stdoutlog, "\ni:%i  j:%i   d={%i,%i,%i}
            //                cm={%f,%f,%f}\n\tp_l_c[i+j]={%i,%i,%i}
            //                p_l_c[i+j-1]={%i,%i,%i}",i,j,s_dx[i*lengthpercore+j],s_dy[i*lengthpercore+j],s_dz[i*lengthpercore+j],centerofmass[0],centerofmass[1],centerofmass[2],poly_lattice_coordinates[3
            //                * (i+j)] ,poly_lattice_coordinates[3 * (i+j)+1]
            //                ,poly_lattice_coordinates[3 *
            //                (i+j)+2],poly_lattice_coordinates[3 *
            //                (i+j-1)],poly_lattice_coordinates[3 * (i+j-1) +
            //                1],poly_lattice_coordinates[3 * (i+j-1)+2]);
            //                fclose(stdoutlog);
        }
        ...
        {
            rsum[i] += (s_dx[i * lengthpercore + j] - centerofmass[0]) *
                           (s_dx[i * lengthpercore + j] - centerofmass[0]) +
                       (s_dy[i * lengthpercore + j] - centerofmass[1]) *
                           (s_dy[i * lengthpercore + j] - centerofmass[1]) +
                       (s_dz[i * lengthpercore + j] - centerofmass[2]) *
                           (s_dz[i * lengthpercore + j] - centerofmass[2]);
            rsum_z[i] += (s_dz[i * lengthpercore + j] - centerofmass[2]) *
                         (s_dz[i * lengthpercore + j] - centerofmass[2]);
            rsum_xy[i] += (s_dx[i * lengthpercore + j] - centerofmass[0]) *
                              (s_dx[i * lengthpercore + j] - centerofmass[0]) +
                          (s_dy[i * lengthpercore + j] - centerofmass[1]) *
                              (s_dy[i * lengthpercore + j] - centerofmass[1]);
    ...
    for (int i = 0; i < numbercores * pertrusionpercore; i++) {
        ...
        radius_z += rr_z;
        radius_xy += rr_xy;
        indiv_rog[index * numbercores * pertrusionpercore + i] +=
            (rr) * (rr) / lengthpercore;
    }

    //        stdoutlog = fopen(stdoutlogname, "a");
    //        fprintf(stdoutlog, "\nradius: %f  radius_z: %f
    //        radius_xy:%f\n",radius,radius_z,radius_xy); fclose(stdoutlog);
    rog[index] += (radius / (numbercores * pertrusionpercore)) *
                  (radius / (numbercores * pertrusionpercore)) / lengthpercore;
    rog_z[index] += (radius_z / (numbercores * pertrusionpercore)) *
                    (radius_z / (numbercores * pertrusionpercore)) /
                    lengthpercore;
    rog_xy[index] += (radius_xy / (numbercores * pertrusionpercore)) *
                     (radius_xy / (numbercores * pertrusionpercore)) /
                     lengthpercore;
         ...
         i++)  // performs the rotations
    {
        ...
        for (int j = 0; j < lengthpercore - 2; j++)  // performs the rotations
        {
            ssum[i] = (((double)(s_vector[3 * (j)])) - s_avg[0]) *
                          (((double)(s_vector[3 * (j)])) - s_avg[0]) +
                      (((double)(s_vector[3 * (j) + 1])) - s_avg[1]) *
                          (((double)(s_vector[3 * (j) + 1])) - s_avg[1]) +
                      (((double)(s_vector[3 * (j) + 2])) - s_avg[2]) *
                          (((double)(s_vector[3 * (j) + 2])) - s_avg[2]);
        }
    }
    ...
    for (int i = (0); i < numbercores * pertrusionpercore * lengthpercore;
         i++)  // performs the rotations
    {
        density_profile[index * L1dim_S_z +
                        (poly_lattice_coordinates[3 * i + 2])]++;
    }
    ...
    if (output_choice)
        output_lattice(index,
                       (radius / (numbercores * pertrusionpercore)) *
                           (radius / (numbercores * pertrusionpercore)) /
                           lengthpercore,
                       tort / (numbercores * pertrusionpercore), dx, dy, dz,
                       fakeDP, fakeDPS, 0);
    ...
    //        int arm = 0;
    //        sprintf(filename,
    //        "Chain_CCords_Proc%i_%i_%i.txt",myid,numbercores,lengthpercore);
    //        if ((file = fopen(filename, "w")) == NULL)
    //        {
    //            stdoutlog = fopen(stdoutlogname, "a");
    //            fprintf(stdoutlog, "Can not open file %s. Exit.\n", filename);
    ...
    //        {
    //            for (int i = 0; i <
    //            numbercores*pertrusionpercore*lengthpercore; i++)
    //            {
    //                summed_diff[0]=poly_lattice_coordinates[3 *
    //                (arm*lengthpercore)];
    //                summed_diff[1]=poly_lattice_coordinates[3 *
    ...
    //
    //            sprintf(filename,
    //            "Solvent_CCords_Proc%i_%i_%i.txt",myid,numbercores,lengthpercore);
    //            if ((file = fopen(filename, "w")) == NULL)
    //            {
    //                stdoutlog = fopen(stdoutlogname, "a");
    //                fprintf(stdoutlog, "Can not open file %s. Exit.\n",
    //                filename); fclose(stdoutlog); MPI_Abort(MPI_COMM_WORLD,
    //                1);
    //            }
    //            else
    //            {
...
void sysstatus(int standard, int(energie), int(eold), int(iteration),
               double(lnf), int(swap_every), double(sweep)) {
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog, "sysstatus OK\n");
    fclose(stdoutlog);

    int counter = 0;

    if (standard <= 0)
        sprintf(resetbuffer, "SysStatus.Initial.L%i.proc%04i", L1dim_S_xy,
                myid);  // Preliminary output file that allows for process
                        // inspection

    if (standard == 1)
        sprintf(resetbuffer, "SysStatus.L%i.proc%04i", L1dim_S_xy,
                myid);  // Preliminary output file that allows for process
                        // inspection

    if (standard == 2)
        sprintf(resetbuffer, "SysStatus.L%i.proc%04i.copy", L1dim_S_xy,
                myid);  // Preliminary output file that allows for process
                        // inspection

//    if (standard == 0) {
//        lll =
//            (long long*)malloc(L1dim_S_z * hist_size * 2.2 * sizeof(long long));
//    }
    ...
    if ((filee = fopen(resetbuffer, "w")) == NULL) {
        ...
        checks_counter[counter] += 0;
        fprintf(filee, "%lld\n", (long long)(.99 * pow(10, 17)));
        if (standard == 0) {
//            lll[checks_counter[counter]] = (long long)(.99 * pow(10, 17));
            checks_counter[counter]++;
        }
//        stdoutlog = fopen(stdoutlogname, "a");
//        fprintf(stdoutlog, "%i : %i\n", counter, checks_counter[counter]);
//        fclose(stdoutlog);
        counter++;

        checks_counter[counter] += checks_counter[counter - 1];
        fprintf(filee, "%i\n", energie);
        if (standard == 0) {
//            lll[checks_counter[counter]] = energie;
            checks_counter[counter]++;
        }
        counter++;
        ...
        fprintf(filee, "%i\n", eold);
        if (standard == 0) {
//            lll[checks_counter[counter]] = eold;
            checks_counter[counter]++;
        }
        counter++;
        ...
        fprintf(filee, "%lld\n", (long long)(sweep));
        if (standard == 0) {
//            lll[checks_counter[counter]] = (long long)(sweep);
            checks_counter[counter]++;
        }
        counter++;
        ...
        fprintf(filee, "%i\n", swap_every);
        if (standard == 0) {
//            lll[checks_counter[counter]] = swap_every;
            checks_counter[counter]++;
        }
        counter++;
        ...
        if (standard == 0) {
//            lll[checks_counter[counter]] = (long long)(lnf * pow(10, 15));
            checks_counter[counter]++;
        }
        counter++;
        ...
        fprintf(filee, "%i\n", iteration);
        if (standard == 0) {
//            lll[checks_counter[counter]] = iteration;
            checks_counter[counter]++;
        }
        counter++;
        ...
        for (int i = 0; i < hist_size; i++) {
            fprintf(filee, "%lld\n", (long long)HE[i]);
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)HE[i];
                checks_counter[counter]++;
            }
        }
        counter++;
        ...
        for (int i = 0; i < hist_size; i++) {
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)(lngE[i] * pow(10, 9));
                checks_counter[counter]++;
            }
        }
        counter++;
        ...
        for (int i = 0; i < hist_size; i++) {
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)(rog[i] * pow(10, 6));
                checks_counter[counter]++;
            }
        }
        stdoutlog = fopen(stdoutlogname, "a");
        ...
        for (int i = 0; i < hist_size; i++) {
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)(tortuosity[i] * pow(10, 6));
                checks_counter[counter]++;
            }
        }
        counter++;
        ...
        for (int i = 0; i < hist_size; i++) {
            fprintf(filee, "%lld\n", (long long)(visits[i]));
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)(visits[i] * pow(10, 6));
                checks_counter[counter]++;
            }
        }
        counter++;
        ...
        for (int i = 0; i < numbercores * pertrusionpercore * lengthpercore;
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)poly_lattice_indexes[i];
                checks_counter[counter]++;
            }
        }
        stdoutlog = fopen(stdoutlogname, "a");
        ...
        for (int i = 0; i < no_solvent_sites; i++) {
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] = (long long)solvent_loc[i];
                checks_counter[counter]++;
            }
        }
        counter++;
        ...
        for (int i = 0; i < hist_size; i++) {
            for (int j = 0; j < L1dim_S_z; j++) {
                ...
                if (standard == 0) {
//                    lll[checks_counter[counter]] =(long long)(density_profile[i * L1dim_S_z + j] *pow(10, 9));
                    checks_counter[counter]++;
                }
            }
        }
        ...
        for (int i = 0; i < hist_size; i++) {
            for (int j = 0; j < L1dim_S_z; j++) {
                ...
                if (standard == 0) {
//                    lll[checks_counter[counter]] = (long long)(density_profile_solvent[i * L1dim_S_z + j] * pow(10, 9));
                    checks_counter[counter]++;
                }
            }
        }
        ...
        for (int i = 0; i < hist_size; i++) {
            for (int i = 0; i < numbercores; i++) {
                ...
                if (standard == 0) {
//                    lll[checks_counter[counter]] = (long long)(indiv_rog[i * numbercores] * pow(10, 6));
                    checks_counter[counter]++;
                }
            }
        }
        ...
        for (int i = 0; i < hist_size; i++) {
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] =(long long)(rog_z[i] * pow(10, 6));
		checks_counter[counter]++;
            }
        }
        counter++;
        ...
        for (int i = 0; i < hist_size; i++) {
            ...
            if (standard == 0) {
//                lll[checks_counter[counter]] =(long long)(rog_xy[i] * pow(10, 6));
                checks_counter[counter]++;
            }
        }
    ...
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog, "sysstatus END\n");
    fclose(stdoutlog);
}

void restart(int(&energie), int(&eold), int(&iteration), double(&lnf), int(&swap_every), double(&sweep)) {
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog, "restart OK\n");
    fclose(stdoutlog);

    int ennergie = total_energy(poly_lattice_coordinates, poly_lattice_indexes,
                                solvent_loc);
    ...
    int stop = 0;
    sprintf(
        resetbuffer, "SysStatus.Initial.L%i.proc%04i", L1dim_S_xy,
        myid);  // Preliminary output file that allows for process inspection
    FILE* rfptr;
    long long phldr;
    ...
    fprintf(stdoutlog, "restart OK\n");
    fclose(stdoutlog);
    ennergie = total_energy(poly_lattice_coordinates, poly_lattice_indexes,
                            solvent_loc);
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog, "Proc %3i: energy at start=%i\n\n", myid, ennergie);
    fclose(stdoutlog);
    ...
    while (fscanf(rfptr, "%lld\n", &phldr) > 0) {
        //    stdoutlog = fopen(stdoutlogname, "a");
        // fprintf(stdoutlog, "restart %i %lld  %i  %i
        // %i\n",resetiter,phldr,checks_counter[0],checks_counter[1],countt0);
        //        fclose(stdoutlog);

        if (resetiter >= 0 && resetiter < checks_counter[0]) {
            // ratiovalue =0.9956;
            
        }
        if (resetiter >= checks_counter[0] && resetiter < checks_counter[1]) {
        
           
            energie = phldr;
        }
        if (resetiter >= checks_counter[1] && resetiter < checks_counter[2]) {
            
            eold = phldr;

        }
        if (resetiter >= checks_counter[2] && resetiter < checks_counter[3]) {
           
            sweep = phldr;
            
        }
        if (resetiter >= checks_counter[3] && resetiter < checks_counter[4]) {
          
            swap_every = phldr;

        }
        if (resetiter >= checks_counter[4] && resetiter < checks_counter[5]) {
           lnf = ((double)phldr) / pow(10, 15);

        }
        if (resetiter >= checks_counter[5] && resetiter < checks_counter[6]) {
            
            iteration = phldr;
            
        }
        if (resetiter >= checks_counter[6] && resetiter < checks_counter[7]) {
            HE[fakeEmin1] = ((int)phldr);
            
            fakeEmin1++;
        }
        if (resetiter >= checks_counter[7] && resetiter < checks_counter[8]) {
            lngE[fakeEmin2] = ((double)phldr) / pow(10, 9);
            fakeEmin2++;
            
        }
        if (resetiter >= checks_counter[8] && resetiter < checks_counter[9]) {
        ...
        if (resetiter >= checks_counter[9] && resetiter < checks_counter[10]) {
            tortuosity[fakeEmin4] = ((double)phldr) / pow(10, 6);
            fakeEmin4++;
            
        }
        if (resetiter >= checks_counter[10] && resetiter < checks_counter[11]) {
            visits[fakeEmin5] = ((long long)phldr);
            fakeEmin5++;
            
        }
        if (resetiter >= checks_counter[11] && resetiter < checks_counter[12]) {
        ...
        if (resetiter >= checks_counter[12] && resetiter < checks_counter[13]) {
            
            solvent_loc[fakeEmin6] = ((int)phldr);
            fakeEmin6++;
           
        }
        if (resetiter >= checks_counter[13] && resetiter < checks_counter[14]) {
            density_profile[fakeEmin7] = ((double)phldr) / pow(10, 9);
            fakeEmin7++;

            
        }
        if (resetiter >= checks_counter[14] && resetiter < checks_counter[15]) {
            density_profile_solvent[fakeEmin8] = ((double)phldr) / pow(10, 9);
            fakeEmin8++;
           
        }
        if (resetiter >= checks_counter[15] && resetiter < checks_counter[16]) {
            indiv_rog[fakeEmin9] = ((double)phldr) / pow(10, 6);
            fakeEmin9++;
            
        }
        if (resetiter >= checks_counter[16] && resetiter < checks_counter[17]) {
            rog_z[fakeEmin10] = ((double)phldr) / pow(10, 6);
            fakeEmin10++;
           
        }
        if (resetiter >= checks_counter[17] && resetiter < checks_counter[18]) {
    ...
    int eenergie = total_energy(poly_lattice_coordinates, poly_lattice_indexes,
                                solvent_loc);
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog, "Proc %3i: energy at start=%i\n", myid, eenergie);
    fclose(stdoutlog);
    // energie -= Eglobalmin; // shift to positive values to use it as array
    // index

    eenergie -= Eglobalmin;
    Estartindex = eenergie;

    stop = 0;
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog,
            "Proc %3i: Parameter: Eminindex -- Emaxindex: %i -- %i; "
            "Estartindex=%i\t",
            myid, Eminindex, Emaxindex, Estartindex);
    fclose(stdoutlog);

    if (eenergie != energie) {
        stdoutlog = fopen(stdoutlogname, "a");
        fprintf(stdoutlog,
                "2nd Estart out of range!!!  energie %i  eeenergie: %i\n",
                energie, eenergie);
        fclose(stdoutlog);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 8) {
        if (myid == 0) {
            ...
            fprintf(stdoutlog, "Expect 4 arguments, %d were provided.\n",
                    argc - 1);
            fprintf(stdoutlog,
                    "Syntax: ./WLpotts_mpi [arg1] [arg2] [arg3] [arg4] \n\n");
            fprintf(stdoutlog,
                    "Please provide the following command line arguments:\n");
            fprintf(stdoutlog,
                    "1. Overlap between consecutive windows. [double, 0 <= "
                    "overlap <= 1]\n");
            fprintf(stdoutlog,
            ...
            printf(
                "1. Overlap between consecutive windows. [double, 0 <= overlap "
                "<= 1]\n");
    ...
    if ((numprocs / multiple) % 2 == 0) {
        if (myid == 0) {
            stdoutlog = fopen(stdoutlogname, "a");
            fprintf(stdoutlog,
                    "ERROR: Even number of energy windows (%d) requested. "
                    "Please request an odd number of energy windows.\n\n",
                    numprocs / multiple);
            fclose(stdoutlog);

            printf(
                "ERROR: Even number of energy windows (%d) requested. Please "
                "request an odd number of energy windows.\n\n",
                numprocs / multiple);
        }
    ...
    int swap_every =
        atoi(argv[3]);  // after this number of sweeps try conformations swap
    ...
    ret_status = find_local_energy_range(Eglobalmin, Eglobalmax, atof(argv[1]),
                                         numprocs);
    ...
    if (ret_status != 0) {
        ...
        MPI_Abort(MPI_COMM_WORLD,
                  1);  // something went wrong in find_local_energy_range()
    }
    ...
    int eold, energie;
    energie = total_energy(poly_lattice_coordinates, poly_lattice_indexes,
                           solvent_loc);
    stdoutlog = fopen(stdoutlogname, "a");
    fprintf(stdoutlog, "Proc %3i: energy at start=%i\n", myid, energie);
    fclose(stdoutlog);
    ...
    MPI_Comm_group(
        MPI_COMM_WORLD,
        &world);  // get the group of processes in MPI_COMM_WORLD (i.e. all)
    int* ranks;
    ranks = (int*)malloc(2 * multiple * sizeof(int));
    mpi_local_group =
        (MPI_Group*)malloc(((numprocs / multiple) - 1) * sizeof(MPI_Group));
    mpi_local_comm =
        (MPI_Comm*)malloc(((numprocs / multiple) - 1) * sizeof(MPI_Comm));

    for (int i = 0; i < ((numprocs / multiple) - 1);
         i++)  // i is counter for the energy range windows
    {
        for (int j = 0; j < 2 * multiple; j++) {
            ranks[j] = i * multiple +
                       j;  // contains the ranks of processes in MPI_COMM_WORLD
                           // which should get into local group
            if (myid == 0) {
                fprintf(stdoutlog,
                        "Proc %3i: %i will be part of communicator/group %i\n",
                        myid, ranks[j], i);
            }
        }
        MPI_Group_incl(world, 2 * multiple, ranks,
                       &mpi_local_group[i]);  // create local group
        MPI_Comm_create(
            MPI_COMM_WORLD, mpi_local_group[i],
            &mpi_local_comm[i]);  // create communicator for that group
    }
    fclose(stdoutlog);
    free(ranks);
    ...
    if (myid < numprocs - multiple) {
        ...
        stdoutlog = fopen(stdoutlogname, "a");
        fprintf(stdoutlog,
                "Proc %3i: I am part of communicator/group %i with "
                "local_id[0]=%i\n",
    ...
    // log 'path'data (the same data is written, just differently sorted: with
    // respect to the sample id and with respect to process id)
    sprintf(wanderlogname, "wanderlog_rseed%i_sample_%i.dat", rseed,
            latticepoint_S[numberspins_S + 2]);
    wanderlog = fopen(wanderlogname, "w");
    time(&timenow);
    fprintf(wanderlog, "#sweep\t\tproc-id\tconf-id\tenergy\t\ttime\n");
    fprintf(wanderlog, "%e\t%i\t%i\t%i\t%s", 0.0, myid,
            latticepoint_S[numberspins_S + 2], energie, ctime(&timenow));
    fclose(wanderlog);
    ...
    int wiggle;
    int wiggletwo;  // ID of spin to be updated (only single-spin update
                    // implemented)
    ...
    double lnf_slowest =
        lnf;  // modification factor of slowest walker in my window
    long long recombine_counter = 0;
    long long recombine_counter_c =
        0;  // modification factor of slowest walker in my window
    ...
    sysstatus(0, energie, eold, iteration, lnf, swap_every, sweep);
   ...
   double counter_iteration=iteration;
    ...
    while (lnf_slowest > lnfmin) {
        ...
        if (restartiter % 200 == 0) {
            ...
            //                //fclose(stdoutlog);
            //
            //                if(array[2] == plane_z_max && latticepoint_S[i] ==
            //                0)
            //                {
            //                    latticepoint_S[i] = -1;
            //                }
            ...
            //
            //            reset_indexes[0] = 0-1;
            //            reset_indexes[1]=numbercores * lengthpercore *
            //            pertrusionpercore;
            //            lattice_poly_index_reset(ph_poly_lattice_indexes,
            //            poly_lattice_indexes, reset_indexes[0],
            //            reset_indexes[1]);
            //                poly_coord_reset(ph_poly_lattice_coordinates,
            //                poly_lattice_coordinates, reset_indexes[0],
            //                reset_indexes[1]);
            //
            //
            //
            ...
            //
            eye();
            eee = total_energy(poly_lattice_coordinates, poly_lattice_indexes,
                               solvent_loc);
            ...
            MPI_Barrier(MPI_COMM_WORLD);
            sysstatus(1, energie, eold, iteration, lnf, swap_every, sweep);
            MPI_Barrier(MPI_COMM_WORLD);
            sysstatus(2, energie, eold, iteration, lnf, swap_every, sweep);
        ...
        for (int k = 0; k < check_flatness_every; k++) {
            for (int i = 0;
                 i < (numbercores * lengthpercore * pertrusionpercore) +
                         no_solvent_sites;
                 i++)  // this does 1 MC sweep
            {
                //                int eeee =
                //                total_energy(poly_lattice_coordinates,poly_lattice_indexes,solvent_loc);
                //                stdoutlog = fopen(stdoutlogname, "a");
                ...
                energie = eold + propose_update();  // calculate energy of
                                                    // updated configuration

                if (MoveProposal == 0 ||
                    ((energie > Emaxindex) ||
                     (energie < Eminindex)))  // boundary check
                {
                    //                    stdoutlog = fopen(stdoutlogname, "a");
                    ...
                    //
                    //                    stdoutlog = fopen(stdoutlogname, "a");
                    //                    for (int i = 0; i < numbercores *
                    //                    lengthpercore * pertrusionpercore;
                    //                    i++)
                    //                    {
                    //                        fprintf(stdoutlog,
                    //                        "%4i\t\t%4i\t%4i\t%4i\t%4i\t\t%4i\t%4i\t%4i\n",
                    //                        poly_lattice_indexes[i],
                    //                        poly_lattice_coordinates[3 * i],
                    //                        poly_lattice_coordinates[3 * i +
                    //                        1], poly_lattice_coordinates[3 * i
                    //                        + 2],ph_poly_lattice_indexes[i],
                    //                        ph_poly_lattice_coordinates[3 *
                    //                        i], ph_poly_lattice_coordinates[3
                    //                        * i + 1],
                    //                        ph_poly_lattice_coordinates[3 * i
                    //                        + 2]);
                    //                    }
                    //                    for (int i = 0; i < no_solvent_sites;
                    //                    i++)
                    //                    {
                    //                        fprintf(stdoutlog,
                    //                        "%4i\t\t%4i\t\t%i\n",
                    ...
                    if (MoveProposal == 1) {
                        if (poly_solvent_mov == 0) {
                            // lattice_polymer_reset(poly_lattice_indexes,
                            // wl_pseudo_chain, reset_indexes[0],
                            // reset_indexes[1]);
                            lattice_poly_index_reset(
                                poly_lattice_indexes, wl_pseudo_chain,
                                reset_indexes[0], reset_indexes[1]);
                            poly_coord_reset(poly_lattice_coordinates,
                                             wl_pseudo_chain_coordinates,
                                             reset_indexes[0],
                                             reset_indexes[1]);
                            // lattice_polymer_reset(ph_poly_lattice_indexes,
                            // poly_lattice_indexes, reset_indexes[0],
                            // reset_indexes[1]);
                            lattice_poly_index_reset(
                                ph_poly_lattice_indexes, poly_lattice_indexes,
                                reset_indexes[0], reset_indexes[1]);
                            poly_coord_reset(ph_poly_lattice_coordinates,
                    ...
                    //                    fclose(stdoutlog);
                    energie = eold;
                    // lngE[energie] = 1; // the real lngE of the system is set
                    // to a value of 1 if visitation occured at a specific energy
                    // (can be subtracted later) to force visitation in the real
                    // wl process
                } else  // calculate acceptance propbability
                {
                    // the regular wl process is kept using a place holder
                    // pseudolngE to force the system to not move to move to
                    // higher energies
                    dice = (1.0 * rand() / (RAND_MAX + 1.0));  // roll the dice
                    wsk = exp(
                        lngE[eold - eold % 10] -
                        lngE[energie - energie % 10]);  // calculate acceptance
                                                        // probability
                    if (dice > wsk)                     // reject
                    {
                        //                        stdoutlog =
                        ...
                        //                                fclose(stdoutlog);
                        if (poly_solvent_mov == 0) {
                            // lattice_polymer_reset(wl_pseudo_chain,
                            // poly_lattice_indexes, reset_indexes[0],
                            // reset_indexes[1]);
                            lattice_poly_index_reset(
                                wl_pseudo_chain, poly_lattice_indexes,
                                reset_indexes[0], reset_indexes[1]);
                            poly_coord_reset(wl_pseudo_chain_coordinates,
                                             poly_lattice_coordinates,
                                             reset_indexes[0],
                                             reset_indexes[1]);
                        }
                        ...
                        if (myid == 0 && energie <= minimum_en_config &&
                            printed == 0) {
                            printed = 1;
                            ...
                            if ((file = fopen(filename, "w")) == NULL) {
                                stdoutlog = fopen(stdoutlogname, "a");
                                fprintf(stdoutlog,
                                        "Can not open file %s. Exit.\n",
                                        filename);
                                fclose(stdoutlog);
                                MPI_Abort(MPI_COMM_WORLD, 1);
                            } else {
                                for (int i = 0;
                                     i < numbercores * pertrusionpercore *
                                             lengthpercore;
                                     i++) {
                                    fprintf(file, "%i\t%i\n", i,
                                            poly_lattice_indexes[i]);
                        ...
                        if (myid == numprocs - 1 &&
                            energie >= maximum_en_config && max_printed == 0) {
                            max_printed = 1;
                        ...
                        //
                        //                    lattice_polymer_reset(wl_pseudo_chain,
                        //                    poly_lattice_indexes,
                        //                    reset_indexes[0],
                        //                    reset_indexes[1]);
                        //                    lattice_poly_index_reset(wl_pseudo_chain,
                        //                    poly_lattice_indexes,
                        //                    reset_indexes[0],
                        //                    reset_indexes[1]);
                        //                    poly_coord_reset(wl_pseudo_chain_coordinates,
                ...
                //                total_energy(poly_lattice_coordinates,poly_lattice_indexes,solvent_loc);
                //                stdoutlog = fopen(stdoutlogname, "a");
                //                fprintf(stdoutlog, "eee: %i   energie:%i
                //                s_or_p:%i   r0:%i
                ...
                //                stdoutlog = fopen(stdoutlogname, "a");
                //                for (int i = 0; i < numbercores *
                //                lengthpercore * pertrusionpercore; i++)
                //                {
                //                    fprintf(stdoutlog,
                //                    "%4i\t\t%4i\t%4i\t%4i\t%4i\t\t%4i\t%4i\t%4i\n",
                //                    poly_lattice_indexes[i],
                ...
                //                    2],ph_poly_lattice_indexes[i],
                //                    ph_poly_lattice_coordinates[3 * i],
                //                    ph_poly_lattice_coordinates[3 * i + 1],
                //                    ph_poly_lattice_coordinates[3 * i + 2]);
                //                }
                //                for (int i = 0; i < no_solvent_sites; i++)
                //                {
            ...
            if (swap_every ==
                0)  // ignition! (time to try to exchange configurations)
            {
                swap_every = swap_every_init;  // reset countdown clock

                if (replica_exchange(sweep / swap_every, eold) ==
                    1)  // if configs were exchanged ('accept'-branch)
                {
                    energie = switched;
                    eold = energie;
                    ...
                    reset_indexes[0] = 0 - 1;
                    reset_indexes[1] = no_solvent_sites;
                    poly_latticepoint_clear(wl_solvent_loc, reset_indexes[0],
                                            reset_indexes[1]);

                    reset_indexes[0] = 0 - 1;
                    reset_indexes[1] =
                        numbercores * lengthpercore * pertrusionpercore;
                    poly_latticepoint_clear(wl_pseudo_chain, reset_indexes[0],
                                            reset_indexes[1]);
                    ...
                    for (int i = 0;
                         ...
                         i++) {
                        wl_pseudo_chain[i] = poly_lattice_indexes[i];
                        wl_pseudo_chain_coordinates[3 * i] =
                            poly_lattice_coordinates[3 * i];
                        wl_pseudo_chain_coordinates[3 * i + 1] =
                            poly_lattice_coordinates[3 * i + 1];
                        wl_pseudo_chain_coordinates[3 * i + 2] =
                    ...
                    // lattice_polymer_reset(wl_pseudo_chain,
                    // poly_lattice_indexes, reset_indexes[0], reset_indexes[1]);
                    // lattice_poly_index_reset(wl_pseudo_chain,
                    // poly_lattice_indexes, reset_indexes[0], reset_indexes[1]);
                    // poly_coord_reset(wl_pseudo_chain_coordinates,
                    // poly_lattice_coordinates, reset_indexes[0],
                    ...
                    stdoutlog = fopen(stdoutlogname, "a");
                    fprintf(stdoutlog,
                            "Proc %3i: %i iteration, %e sweeps, Replica "
                            "Exchange Success\n Energy Calculted: %i \t "
                            "Recievced Index Energy: %i\n",
                            myid, iteration, sweep, energie, switched);
                    fclose(stdoutlog);
                }
                // update histograms (independently of whether RE happened or
                // not)
                lngE[energie - energie % 10] += lnf;
                HE[energie - energie % 10]++;
            }
        ...
        sprintf(filename, "Prelim.L%iq%i.HE.proc%04i.iter%i", L1dim_S_xy, q,
                myid, iteration);  // Preliminary output file that allows for
                                   // process inspection
        if ((file = fopen(filename, "w")) == NULL) {
            stdoutlog = fopen(stdoutlogname, "a");
            fprintf(stdoutlog, "Can not open file %s. Exit.\n", filename);
            ...
            for (int i = Eminindex; i <= Emaxindex; i++) {
                if (lngE[i] > 0.5) {
                    fprintf(file, "%i\t%i\t%e\t%e\t%e\t%f\t%f\n", i,
                            (i + (Eglobalmin)), 0.0, lngE[i], HE[i],
                            rog[i] / visits[i], tortuosity[i] / visits[i]);
                }
            }
            fclose(file);
        }
        ...
        if (flat ==
            0)  // histogram not flat, do nothing except maybe some logging
        {
            stdoutlog =
                fopen(stdoutlogname, "a");  // Prints out effictively flatness
                                            // progress for inspection
            fprintf(stdoutlog,
                    "Proc %3i: %i iteration, %e sweeps, Histogram noch nicht "
                    "flach\nValue of Tried Minimum Histogram Value: %f    "
                    "Value of Tried Ratio: %f\n",
                    myid, iteration, sweep, flatmin, flatratio);

            fclose(stdoutlog);
            // phrepv = flatmin; // holds the minimum value of the histogram and
            // checks change and prints out the current lattice if the value has
            // reoccured frequently if (repv == phrepv)
            //{
            //    iterr++;
            //    repv = phrepv;
            //}
        ...
        {
            if (lnf > lnfmin)  // as long as modification factor is still larger
                               // than terminal value
            {
                sprintf(filename, "L%iq%i.HE.proc%04i.iter%i", L1dim_S_xy, q,
                        myid, iteration);
                if ((file = fopen(filename, "w")) == NULL) {
                    stdoutlog = fopen(stdoutlogname, "a");
                    fprintf(stdoutlog, "Can not open file %s. Exit.\n",
	...
	for (int y = (0); y < chain_length - 2; y++)	// performs the rotations
	{
		ssum += (((double)(s_vector[3 *y])) - s_avg[0]) *(((double)(s_vector[3 *y])) - s_avg[0]) + (((double)(s_vector[3 *y + 1])) - s_avg[1]) *(((double)(s_vector[3 *y + 1])) - s_avg[1]) + (((double)(s_vector[3 *y + 2])) - s_avg[2]) *(((double)(s_vector[3 *y + 2])) - s_avg[2]);
            ...
            if (merge_hists == 1)  // merge g(E) estimators from multiple
                                   // walkers in the same energy window
            {
                stdoutlog = fopen(stdoutlogname, "a");
                if (myid % multiple ==
                    0)  // 'root' in energy window, receive individual g(E) and
                        // send merged g(E)
                {
                    for (int i = 1; i < multiple; i++) {
                        MPI_Recv(&lngE_buf[0], hist_size, MPI_DOUBLE, myid + i,
                                 77, MPI_COMM_WORLD,
                                 &status);  // get other dens. of states
                        fprintf(stdoutlog,
                    ...
                    for (int j = 0; j < hist_size; j++)
                        lngE[j] /= (double)multiple;  // normalize
                    for (int i = 1; i < multiple; i++) {
                        MPI_Send(&lngE[0], hist_size, MPI_DOUBLE, myid + i, 99,
                                 MPI_COMM_WORLD);
                        fprintf(stdoutlog,
                                "Proc %i: Sent merged lngE to Proc. %i\n", myid,
                                myid + i);
                    }
                } else  // send individual g(E) and receive merged g(E)
                {
                    MPI_Send(&lngE[0], hist_size, MPI_DOUBLE,
                             myid - (myid % multiple), 77, MPI_COMM_WORLD);
                    fprintf(stdoutlog, "Proc %i: Sent lngE to Proc. %i\n", myid,
                            myid - (myid % multiple));
                    MPI_Recv(&lngE_buf[0], hist_size, MPI_DOUBLE,
                             myid - (myid % multiple), 99, MPI_COMM_WORLD,
                             &status);
                    fprintf(stdoutlog,
                            "Proc %i: Received merged lngE from Proc. %i\n",
                            myid, myid - (myid % multiple));
                    for (int j = 0; j < hist_size; j++)
                        lngE[j] = lngE_buf[j];  // replace individual lngE
                                                // (could be done directly, yes)
                }
                fclose(stdoutlog);
            }
        }
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Allreduce(&lnf, &lnf_slowest, 1, MPI_DOUBLE, MPI_MAX,
                      MPI_COMM_WORLD);
        MPI_Allreduce(&recombine_counter, &recombine_counter_c, 1,
                      MPI_LONG_LONG, MPI_MAX, MPI_COMM_WORLD);

        if (lnf_slowest <=
                (((double)(10000000000 / countdown)) * .0000000001) ||
            recombine_counter_c > 20) {
            MPI_Barrier(MPI_COMM_WORLD);
        ...
        if (flat == 1) {
            stdoutlog = fopen(stdoutlogname, "a");
            fprintf(stdoutlog,
                    "Proc %3i: Start %i iteration, %e sweeps total so far, lnf "
                    "now %e (lnfmin=%.2e, lnf_slowest=%e)\n",
                    myid, iteration, sweep, lnf, lnfmin, lnf_slowest);
            fprintf(
                stdoutlog,
                "Proc %3i: tryleft: %i, exchangeleft %i (Akzeptanzleft:%.2lf) "
                "<--> tryright: %i, exchangeright %i (Akzeptanzright:%.2lf)\n",
                myid, tryleft, exchangeleft,
                (double)exchangeleft / (double)tryleft, tryright, exchangeright,
                (double)exchangeright / (double)tryright);
            fclose(stdoutlog);
        }
